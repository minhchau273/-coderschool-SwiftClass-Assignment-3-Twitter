...
class TwitterHelper: NSObject {
    ...
    func displayImages(tweet: Tweet, imagesView: UIView, imagesViewConstraints: [NSLayoutConstraint!]) {
        ...
        case 1:
            ...
            break
        case 2:
            var imagesViewTmp = NSBundle.mainBundle().loadNibNamed("ImagesView2", owner: self, options: nil).first! as! ImagesView2
            imagesViewTmp.images = tweet.images
            ...
            break
        case 3:
            var imagesViewTmp = NSBundle.mainBundle().loadNibNamed("ImagesView3", owner: self, options: nil).first! as! ImagesView3
            imagesViewTmp.images = tweet.images
            imagesViewTmp.imageView1.setImageWithURL(tweet.images[0])
            imagesViewTmp.imageView2.setImageWithURL(tweet.images[1])
            ...
            break
        case 4:
            var imagesViewTmp = NSBundle.mainBundle().loadNibNamed("ImagesView4", owner: self, options: nil).first! as! ImagesView4
            imagesViewTmp.images = tweet.images
            imagesViewTmp.imageView1.setImageWithURL(tweet.images[0])
            imagesViewTmp.imageView2.setImageWithURL(tweet.images[1])
            imagesViewTmp.imageView3.setImageWithURL(tweet.images[2])
        ...
        if selectedTweet.isFavorited {
            TwitterClient.sharedInstance.unfavoriteTweet(selectedTweet.id!, completion: { (response, error) -> () in
                if response != nil {
                    ...
                    var favoriteCount = selectedTweet.favoriteCount! - 1
    ...
    func tapImage(view: UIView, images: [NSURL], index: Int) {
        ...
        var photoVC = view.parentViewController!.storyboard?.instantiateViewControllerWithIdentifier("photoVC") as! PhotoViewController
